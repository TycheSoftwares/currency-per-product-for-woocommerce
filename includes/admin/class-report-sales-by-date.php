 ...
 *
 * Custom WC_Report_Sales_By_Date for Currency per Product.
 *
 ...
 *
 * Overrides WooCommerce's built-in sales-by-date report so that orders placed
 * in a non-default currency are accounted for correctly.
 */
class WC_Report_Sales_By_Date extends WC_Admin_Report {
	...
	private function query_report_data() {
		...
		$this->report_data->orders = apply_filters(
			'alg_wc_cpp_report_sales_by_date_order',
			(array) $this->get_order_report_data(
				array(
					'data'         => array(
						'_order_total'        => array(
							'type'     => 'meta',
		...
		// If an order is 100% refunded look at the parent's totals, but use the refund dates.
		// Ensure each parent order's values are only counted/summed once.
		...
		// Partial refunds — includes line items, shipping and taxes. Not grouped by date.
		...
		foreach ( $this->report_data->refunded_orders as $key => $value ) {
			...
			// Only applies to partial refunds.
	...
	public function get_export_button() {
		...
		<a
			href="#"
			download="report-<?php echo esc_attr( $current_range ); ?>-<?php echo esc_attr( date_i18n( 'Y-m-d', current_time( 'timestamp' ) ) ); // phpcs:ignore ?>.csv"
			class="export_csv"
			data-export="chart"
				...
				var drawGraph = function( highlight ) {
					var series = [
						...
						},
						{
							...
							lines: { show: true, lineWidth: 2, fill: false },
							shadowSize: 0,
							prepend_tooltip: "<?php echo get_woocommerce_currency_symbol(); // phpcs:ignore ?>"
						},
						{
