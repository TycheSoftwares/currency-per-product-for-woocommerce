	...
	function get_settings() {
		...
		$currencies_general_settings = array(
			...
			array(
				...
				'desc'     => sprintf( __( 'Currency per %s', 'currency-per-product-for-woocommerce' ),
		...
		if ( $currency_from != get_option( 'alg_wc_cpp_currency_' . '0', $currency_from ) ) {
			update_option( 'alg_wc_cpp_currency_' . '0', $currency_from );
		}
		$currencies_settings = array(
			array(
				...
				'desc'     => sprintf(
						...
						'<code>' . $currency_from . '</code>' ) . '<br>' .
					sprintf( __( 'If you are missing some currencies (or cryptocurrencies) in the currencies list below - we suggest checking %s plugin.', 'currency-per-product-for-woocommerce' ),
