...
class Alg_WC_CPP_Settings_Exchange_Rates extends Alg_WC_CPP_Settings_Section {
	...
	function get_settings() {
		...
		$update_rates_cron_time = ( 'auto' === apply_filters( 'alg_wc_cpp', 'manual', 'value_exchange_rate_update' ) && 0 != ( $_time = get_option( 'alg_wc_cpp_exchange_rate_cron_time', 0 ) ) ?
			sprintf( '<p>' . __( '%s until next update', 'currency-per-product-for-woocommerce' ) . '.</p>', human_time_diff( $_time ) ) : '' );
		$exchange_rate_update_settings = array(
			array(
				...
				'desc'     => sprintf(
					...
					'<ol>' .
						'<li>' .
							sprintf( __( 'at least one of the behaviour options in %s is set to one of "Convert to ..." options, or', 'currency-per-product-for-woocommerce' ),
						...
						'</li>' .
						'<li>' .
							sprintf( __( '"Sorting by converted price" or "Filtering by converted price" option is enabled in %s.', 'currency-per-product-for-woocommerce' ),
		...
		$exchange_rate_settings = array(
			...
			array(
				...
				'id'       => 'alg_wc_cpp_exchange_rate_' . '0',
				'default'  => 1,
				'type'     => 'number',
		...
		for ( $i = 1; $i <= $total_number; $i++ ) {
			...
			$exchange_rate_settings = array_merge( $exchange_rate_settings, array(
				array(
					...
					'default'  => 1,
					'type'     => 'number',
					'custom_attributes' => array( 'step' => '0.000000000001', 'min'  => '0' ),
				),
			) );
		}
		$exchange_rate_settings = array_merge( $exchange_rate_settings, array(
