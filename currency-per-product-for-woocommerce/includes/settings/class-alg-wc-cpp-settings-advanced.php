		...
		return array(
			...
			array(
				...
				'desc'     => '<table class="widefat striped"><tr><th>' . __( 'Tool', 'currency-per-product-for-woocommerce' ) . '</th><th>' . __( 'Description', 'currency-per-product-for-woocommerce' ) . '</th></tr><tr>' .
					implode( '</tr><tr>', array(
						sprintf( '<td><a class="button" href="' . wp_nonce_url( add_query_arg( 'alg_wc_cpp_calculate_all_products_prices', true,
							admin_url( 'admin.php?page=wc-settings&tab=alg_wc_cpp&section=advanced' ) ), 'alg_wc_cpp_calculate_all_products_prices', 'alg_wc_cpp_calculate_all_products_prices_nonce' ) . '">' .
							...
							'<td><em>' . __( 'Re-calculates (i.e. converts) and saves all products prices for use in "Sort by price" sorting and "Filter Products by Price" widget.', 'currency-per-product-for-woocommerce' ) . ' ' .
								__( 'This is also done automatically: a) when product is saved, b) on scheduled exchange rates updates, and c) periodically%s.', 'currency-per-product-for-woocommerce' ) . ' ' .
								__( 'However you can also do this manually by pressing the button.', 'currency-per-product-for-woocommerce' ) . '</em></td>',
									( ( 'yes' === get_option( 'alg_wc_cpp_sort_by_converted_price', 'no' ) || 'yes' === get_option( 'alg_wc_cpp_filter_by_converted_price', 'no' ) ) &&
										0 != ( $_time = get_option( 'alg_wc_cpp_calculate_all_products_prices_cron_time', 0 ) ) ?
											sprintf( ' (' . __( '%s until next update', 'currency-per-product-for-woocommerce' ) . ')', human_time_diff( $_time ) ) : '' )
						),
						'<td><a class="button" style="color:red;" onclick="return confirm(\'' . __( 'Are you sure?', 'currency-per-product-for-woocommerce' ) . '\')" href="' . wp_nonce_url( add_query_arg( 'alg_wc_cpp_delete_plugin_data', true,
							admin_url( 'admin.php?page=wc-settings&tab=alg_wc_cpp&section=advanced' ) ), 'alg_wc_cpp_delete_plugin_data', 'alg_wc_cpp_delete_plugin_data_nonce' ) . '">' .
